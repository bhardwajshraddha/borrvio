...
const ItemDetail = () => {
  ...
  const fetchWishlistStatus = useCallback(async () => {
    ...
    try {
      ...
      const exists = data.some(
        (entry) => entry.item && entry.item._id === id
      );
  ...
  const toggleWishlist = async () => {
    ...
    try {
      if (wishlisted) {
        await axios.delete(
          `https://borrvio.onrender.com/api/wishlist/${id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        setWishlisted(false);
            ...
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="w-10 h-10 glass rounded-full flex items-center justify-center border border-white/10 hover:border-red-400 transition hover:scale-110"
                >
                  {wishlisted ? (
